...
#       is the mean negative binomial regression deviance evaluated on that set.
...
cv_nb_grpreg = function(Y, X, groups, nb_size=1, penalty=c("gLASSO","gSCAD","gMCP"),
                        n_folds=10, group_weights, taper, 
                        n_lambda=100, lambda, max_iter=10000, tol=1e-4) {
  ...
  ## Check that J is less than or equal to (n_folds-1)/n_folds * n
  if(p > (n_folds-1)/n_folds*n){
    stop("For cross-validation in group-regularized negative binomial regression, we require the 
         total number of covariates to be less than or equal to sample size*(n_folds-1)/n_folds. 
  ...
  if(missing(lambda)) {
    ...
    for(g in 1:G){
      ...
      X_g = X[,ind_g]
      norms_vec[g] = sqrt(sum((t(X_g)%*%Y)^2))/sqrt(length(ind_g))  
    }
  ...
  for(k in 1:n_folds){
    ...
    nb_mod_train = nb_grpreg(Y=Y_train, X=X_train, groups=group_numbers, penalty=penalty,  
                             X_test=X_val, nb_size=nb_size,
  ...
  ## Return a list
 cv_nb_grpreg_output <- list(lambda=lambda,
                             cve=cve,
                             cvse=cvse,
                             lambda_min=lambda_min,
                             min_index=min_index)
  # Return list
  return(cv_nb_grpreg_output)
}