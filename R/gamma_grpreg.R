...
# X_test = n_test x p design matrix for test data. If missing, then the program sets X_test=X
...
#           "gSCAD" for group SCAD, and "gMCP" for group MCP. Negative binomial regression with
...
gamma_grpreg = function(Y, X, groups, X_test, gamma_shape=1, penalty=c("gLASSO","gSCAD","gMCP"), 
  ...
  ## Check that G is less than or equal to n
  if(p > n) {
  ...
  if(!missing(lambda)) {
    n_lambda = length(lambda) # Override nlambda with the length of lambda
  ...
  glm_mod = stats::glm(Y~X, family=stats::Gamma(link="log"))
  gamma_MLE = glm_mod$coefficients # MLE of regression coefficients 
  ...
  Y_check = sqrt_Sigma_inv %*% gamma_MLE
  ...
  ## LSA approach
  if(n_lambda > 1){
        gamma_group = grpreg::grpreg(X=sqrt_Sigma_inv, y=Y_check, group=groups_MLE, penalty=penalty, 
  ...
  beta0 = as.numeric(gamma_group$beta[2,]) 
  
  ## Estimated regression coefficients
  beta = as.matrix(gamma_group$beta[-c(1,2),])
  colnames(beta) = NULL
  ...
  if(return_GIC==TRUE){
    ## Compute GIC
    gammareg_dev = rep(0, L)
    GIC = rep(0, L)
    for(l in 1:L){
      # Compute deviance first
      mu = exp(beta0[l]+X%*%beta[,l])
      gammareg_dev[l] = 2*sum(-log(Y/mu)+(Y-mu)/mu)
      # Compute GIC
      GIC[l] = gammareg_dev[l]/n + (log(log(n))*log(p)*length(which(beta[,l]!=0)))/n
    }
  ...
  ## Return a list
  if(return_GIC==TRUE){
    gamma_grpreg_output <- list(lambda=lambda,
                                beta=beta,
                                beta0=beta0,
